...
def main(arguments):
    ...
    exp_name = "imagenet_ensemble_comparison_" + args.noise_type
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        attack_obj = GradientDescentDL(sess, [ensemble], alpha, data_dims, box_vals, targeted=target_bool, batch_size=1,
                                       max_iterations=opt_iters, learning_rate=lr, confidence=0)

        noise_func = partial(gradientDescentFunc, attack=attack_obj)
        targeted = Target_exp if target_bool else False

        V = adversary(np.array([1.0]), [ensemble], X_exp, Y_exp, alpha, noise_func, targeted)
        np.save(exp_name + ".npy", V)
        log.debug("Success")
